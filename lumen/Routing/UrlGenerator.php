...
class UrlGenerator
{
    ...
    protected function removeIndex($root)
    {
        ...
        return wp_pluginner_str_contains($root, $i) ? str_replace('/'.$i, '', $root) : $root;
    }
    ...
    public function route($name, $parameters = [], $secure = null)
    {
        ...
        $uri = preg_replace_callback('/\{(.*?)(:.*?)?(\{[0-9,]+\})?\}/', function ($m) use (&$parameters) {
            return isset($parameters[$m[1]]) ? wp_pluginner_array_pull($parameters, $m[1]) : $m[0];
        }, $uri);
    ...
    {
        if (wp_pluginner_starts_with($path, ['#', '//', 'mailto:', 'tel:', 'http://', 'https://'])) {
            return true;
        }
    ...
    protected function getRootUrl($scheme, $root = null)
    {
        ...
        $start = wp_pluginner_starts_with($root, 'http://') ? 'http://' : 'https://';
